...
if (!$question_id) die("Question ID not found.");

$question_id = intval($question_id);
$instructor_id = $_SESSION['user']['user_id'];

// Validate ownership + fetch question + quiz info
$q = mysqli_fetch_assoc(mysqli_query($conn, "
  SELECT qs.*, qz.quiz_id, qz.title AS quiz_title, qz.course_id, c.title AS course_title
  FROM questions qs
...
"));
if (!$q) die("Access denied. This question is not assigned to you.");

// Fetch choices
$options = mysqli_query($conn, "SELECT * FROM choices WHERE question_id = $question_id");
if (!$options) die("Query failed: " . mysqli_error($conn));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    header("Location: manage_questions.php?quiz_id=" . $q['quiz_id'] . "&success=1");
    exit;
}
?>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-slate-500">
          Course: <span class="font-semibold text-slate-700"><?= htmlspecialchars($q['course_title']); ?></span>
          • Quiz: <span class="font-semibold text-slate-700"><?= htmlspecialchars($q['quiz_title']); ?></span>
        </p>
      </div>
  ...
  <main class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
      <div class="p-6 border-b border-slate-200">
        ...
        <p class="text-sm text-slate-500">Update the question and select the correct option.</p>
      </div>
        ...
        <form method="POST" class="space-y-6">
          ...
          <div>
            ...
            <textarea name="question_text" rows="4" required
              ...
              placeholder="Type the question here..."><?= htmlspecialchars($q['question_text']); ?></textarea>
          </div>
